...
contract BycClub {
    ...
    function checkOut(address walletAddress) public isRenter(walletAddress) {
        require(renters[walletAddress].due == 0, "You have some balance due.");
    ...
    function checkIn(address walletAddress) public isRenter(walletAddress) {
        require(renters[walletAddress].active == true, "Please check out a ride first.");
        renters[walletAddress].active = false;
        renters[walletAddress].end = block.timestamp;
        setDue(walletAddress);
    }

    // Get total duration of bicycle use
    ...
    function setDue(address walletAddress) internal {
        ...
        uint oneMinuteIncrements = timespanMinutes / 1;
        renters[walletAddress].due = oneMinuteIncrements * 1000000000000000;
    }
    // Check if rent Bicycle
    function canRentBicycle(address walletAddress) public isRenter(walletAddress) view returns(bool) {
    ...
    // Make a Payment
    function Payment(address walletAddress, uint amount) public isRenter(walletAddress) {
